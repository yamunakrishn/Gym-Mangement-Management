...
def user_signup(request):
    error = ""
    if request.method == 'POST':
        # n = request.POST['memname']
        e = request.POST['email']
        a = request.POST['age']
        c = request.POST['contactno']
        u = request.POST['uname']
        p = request.POST['pwd']
        user = authenticate(username=u, password=p, emailid=e,  age=a, contactno=c)
        try:
            if user.is_staff:
                login(request, user)
                error = "no"
...
def trainerlogin(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "yes"
    return render(request,'trainer_login.html', locals())
...
def trainersignup(request):
    ...
    if request.method == 'POST':
        ...
        u = request.POST['uname']
        p = request.POST['pwd']
        user = authenticate(username=u, password=p, emailid=e, contactno=c)
        try:
            if user.is_staff:
                login(request, user)
                error = "no"
            ...
            error = "yes"
    return render(request,'trainer_signup.html', locals())


def taddMember(request):
    if not request.user.is_authenticated:
        return redirect(taddMember)
    error = ""
    plan1 = Plan.objects.all()
    if request.method == "POST":
        n = request.POST['name']
        ...
        try:
            ...
            error = "yes"
    d = {'error':error,'plan':plan1}
    return render(request,'taddMember.html', d)


def tviewMember(request):
    if not request.user.is_authenticated:
        return redirect(admin_login)
    member = Member.objects.all()
    return render(request,'tviewMember.html', locals())
...
def teditMember(request,pid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            error = "yes"
    return render(request,'teditMember.html', locals())
